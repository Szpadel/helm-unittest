...
if [[ $CI == 'true' ]]; then
  ...
  pluginLatest=$(curl --proto "=https" -sL -H "Authorization: token ${GITHUB_TOKEN}"  https://api.github.com/repos/${pluginRepo}/tags?per_page=2 |jq -r ".[].name"|sed 's/^v//'|sort -V |grep -v -)
else
  helmLatest=$(curl --proto "=https" -sL https://api.github.com/repos/${helmRepo}/tags?per_page=1 |jq -r ".[].name"| sed 's/^v//'| sort -V |grep -v -)
  pluginLatest=$(curl --proto "=https" -sL https://api.github.com/repos/${pluginRepo}/tags?per_page=1 |jq -r ".[].name"| sed 's/^v//'| sort -V |grep -v -)
fi
...
do
  ...
  do
    ...
    echo $tag 
    status=$(curl --proto "=https" -sL https://hub.docker.com/v2/repositories/${image}/tags/${tag})
    echo $status
...
helmLatestRelease=$(curl --proto "=https" -s https://github.com/${helmRepo}/releases)
helmLatestVersion=$(echo $helmLatestRelease\" |grep -oP '(?<=tag\/v)[0-9][^"]*'|grep -v \-|sort -Vr|head -1)
pluginLatestRelease=$(curl --proto "=https" -s https://github.com/${pluginRepo}/releases)
